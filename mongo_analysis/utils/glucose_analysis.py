...
class GlucoseAnalysis:
    ...
    def calculate_summary(self): 
        ...
        return {
            'mean': round(float(df['sgv'].mean()), 3),
            'median': round(float(df['sgv'].median()), 3),
            'min': round(float(df['sgv'].min()), 3),
            'max': round(float(df['sgv'].max()), 3),
            'q1': round(float(df['sgv'].quantile(0.25)), 3),
            'q3': round(float(df['sgv'].quantile(0.75)), 3),
            'interday_cv': round(float(self.calculate_interday_cv()), 3),
            'interday_sd': round(float(self.calculate_interday_sd()), 3),
            'intraday_cv_mean': round(float(intraday_cv_mean), 3),
            'intraday_cv_median': round(float(intraday_cv_median), 3),
            'intraday_cv_sd': round(float(intraday_cv_std), 3),
            'intraday_sd_mean': round(float(intraday_sd_mean), 3),
            'intraday_sd_median': round(float(intraday_sd_median), 3),
            'intraday_sd_sd': round(float(intraday_sd_std), 3),
            'tir': round(float(tir), 3),
            'tor': round(float(tor), 3),
            'pir': round(float(self.calculate_pir(total_time, tir)), 3),
            'por': round(float(self.calculate_por(total_time, tor)), 3),
            'mge': round(float(self.calculate_mge(up, dw)), 3),
            'mgn': round(float(self.calculate_mgn(up, dw)), 3),
            'gvi': round(float(self.calculate_gvi()), 3),
            'pgs': round(float(self.calculate_pgs()), 3)
        }
